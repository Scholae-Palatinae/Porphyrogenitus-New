#pragma once
//#define NOMINMAX, in case the setting inside the properties, under preprocessor don't work
#include <functional>
#include <algorithm>
#include <vector>
#include <string>
...
struct SelectorParams
{
	...
	SelectorParams(int x = 20, int y = 10, int columns = 1, int current_x = 0, int current_y = 0,
...
inline Selector<T>::Selector(Console& console, Keyboard& keyboard, std::function<void(int, std::vector<T>)> on_selection,
	...
	: m_Console(console), m_Keyboard(keyboard), m_OnSelection(on_selection), m_OnDrawItem(on_draw_item), m_Data(data), m_Params(params), m_bShowCursor(true)
{
